        ...
        if (fc.run () == Gtk.ResponseType.ACCEPT) {
            ...
            if (file != null) {
                ...
                if (current.name == current_username) {
                    cmd_sync ("bash -c 'cp %s %s && chmod 0644 %s'".printf (
                        Shell.quote (file), Shell.quote (target), Shell.quote (target)));
                    cmd_sync (dbus_cmd);
                } else {
                    cmd_auth ("bash -c 'cp %s %s && chown %s %s && chmod 0644 %s'".printf (
                        Shell.quote (file), Shell.quote (target), Shell.quote (current.name), Shell.quote (target), Shell.quote (target)));
                    cmd_auth (dbus_cmd);
                }
                string saved = current.name;
                reload_users ();
        ...
        if (d.run () == Gtk.ResponseType.OK) {
            ...
            if (cmd_auth (cmd)) {
                ...
                if (file != null) {
                    ...
                    string uid_o = cmd_sync ("id -u " + Shell.quote (e_user.text));
                    if (uid_o != null) {
                        cmd_auth ("busctl call org.freedesktop.Accounts /org/freedesktop/Accounts/User%s org.freedesktop.Accounts.User SetIconFile s %s".printf (uid_o, Shell.quote (target)));
                    }
                }
