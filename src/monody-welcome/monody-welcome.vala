...
public class WelcomeWindow : Gtk.Window {
    ...
    private Gtk.CheckButton opt_gestures;
    ...
    private Gtk.RadioButton browser_keep;
    private Gtk.RadioButton browser_floorp;
    private Gtk.RadioButton browser_librewolf;
    private Gtk.RadioButton browser_firefox;
    private Gtk.RadioButton browser_brave;
    private Gtk.RadioButton browser_helium;
    ...
    public WelcomeWindow () {
        ...
        stack.add_named (build_browser_page (), "browser");
    ...
    private Gtk.Widget build_options_page () {
        ...
        opt_full_firmware = add_option (list,
                                        "Reinstall Stripped Packages",
                                        "Restores the complete linux-firmware and full Mint-L icon packs.",
                                        "drive-harddisk-symbolic");

        opt_gestures = add_option (list,
                                   "Touchpad Gestures",
                                   "Installs touchegg for advanced multi-touch touchpad gestures.",
    ...
    private Gtk.Widget build_browser_page () {
        ...
        var title = new Gtk.Label ("Choose Your Browser");
        title.halign = Gtk.Align.START;
        title.get_style_context ().add_class ("section-title");
        outer.pack_start (title, false, false, 0);

        var sub = new Gtk.Label ("Badwolf is installed by default to keep the ISO small. You can replace it with a full-featured browser now (or later, we don't judge).");
        sub.halign = Gtk.Align.START;
        sub.wrap = true;
        sub.margin_top = 4;
        sub.margin_bottom = 24;
        ...
        list.row_activated.connect ((row) => {
            ...
            var children = box.get_children ();
            foreach (var child in children) {
                if (child is Gtk.RadioButton) {
                    var rb = (Gtk.RadioButton) child;
                    rb.active = true;
                    break;
                }
            }
        ...
        browser_keep = add_radio (list, null, "Keep Badwolf", "Stick with the default browser", "web-browser-symbolic");
        browser_floorp = add_radio (list, browser_keep, "Floorp", "A Firefox fork with more customization and Web App support", "web-browser-symbolic");
        browser_librewolf = add_radio (list, browser_keep, "LibreWolf", "Privacy-focused Firefox fork", "web-browser-symbolic");
        browser_firefox = add_radio (list, browser_keep, "Firefox", "The standard Mozilla Firefox browser", "web-browser-symbolic");
        browser_brave = add_radio (list, browser_keep, "Brave", "Privacy-focused Chromium-based browser with many crypto features", "web-browser-symbolic");
        browser_helium = add_radio (list, browser_keep, "Helium", "Privacy-focused Chromium fork", "web-browser-symbolic");

        outer.pack_start (list, true, true, 0);
        return outer;
    }

    private Gtk.RadioButton add_radio (Gtk.ListBox list, Gtk.RadioButton? group, string name, string desc, string icon_name) {
        var row = new Gtk.ListBoxRow ();
        var box = new Gtk.Box (Gtk.Orientation.HORIZONTAL, 16);
        box.margin = 16;
        ...
        var radio = new Gtk.RadioButton.from_widget (group);
        radio.valign = Gtk.Align.CENTER;
        radio.can_focus = false;
        box.pack_end (radio, false, false, 0);
    ...
    private void on_skip_clicked () {
        if (stack.visible_child_name == "options") {
            stack.visible_child_name = "browser";
        } else if (stack.visible_child_name == "browser") {
            stack.visible_child_name = "utilities";
            action_btn.label = "Next";
    ...
    private string[] build_pkgs () {
        ...
        if (opt_full_firmware.active) {
            res += "linux-firmware"; res += "mint-l-icons"; res += "mint-l-theme";
        }
        if (opt_gestures.active) {
            res += "touchegg";
        }
        ...
        if (browser_floorp != null && browser_floorp.active) {
            res += "floorp-bin";
        }
        if (browser_librewolf != null && browser_librewolf.active) {
            res += "librewolf-bin";
        }
        if (browser_firefox != null && browser_firefox.active) {
            res += "firefox";
        }
        if (browser_brave != null && browser_brave.active) {
            res += "brave-bin";
        }
        if (browser_helium != null && browser_helium.active) {
            res += "helium-browser-bin";
        }
        return res;
    }
    ...
    private void start_install (string[] pkgs) {
        ...
        string cmd = "paru -Syu --noconfirm --needed --ask 4 " + pkgs_str;
        if (browser_keep != null && !browser_keep.active) {
            string browser_exec = "";
            if (browser_floorp != null && browser_floorp.active) browser_exec = "floorp";
            if (browser_librewolf != null && browser_librewolf.active) browser_exec = "librewolf";
            if (browser_firefox != null && browser_firefox.active) browser_exec = "firefox";
            if (browser_brave != null && browser_brave.active) browser_exec = "brave";
            if (browser_helium != null && browser_helium.active) browser_exec = "helium";

            cmd += " && sudo pacman -Rns --noconfirm badwolf || true";
            if (browser_exec != "") {
                cmd += " && sudo sed -i \"s/exec='badwolf'/exec='" + browser_exec + "'/g\" /usr/share/glib-2.0/schemas/99_monody-desktop.gschema.override";
                cmd += " && sudo glib-compile-schemas /usr/share/glib-2.0/schemas/";
            }
        }
        if (opt_flatpak.active) {
            cmd += " && pkexec flatpak remote-add --if-not-exists flathub https://dl.flathub.org/repo/flathub.flatpakrepo";
        }
